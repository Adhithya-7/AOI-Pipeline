...
_OG_MODEL_SRC = r"D:\MELSS\AOI\runs\pcb_finetune_v6\weights\best.pt"
_OG_MODEL_DST = os.path.join(DATA_ROOT, "models", "best.pt")

def _resolve_og_model() -> str:
    """Locate (and if necessary migrate) the original YOLO model.

    Priority order:
      1. Already present at DATA_ROOT/models/best.pt   → use as-is.
      2. Original source path exists (D:\\MELSS\\...)   → copy there first.
      3. Bundled inside the exe (PyInstaller data)     → use bundled path.
      4. Nothing found                                 → return "".
    """
    if os.path.exists(_OG_MODEL_DST):
        return _OG_MODEL_DST
    if os.path.exists(_OG_MODEL_SRC):
        os.makedirs(os.path.dirname(_OG_MODEL_DST), exist_ok=True)
        try:
            shutil.copy2(_OG_MODEL_SRC, _OG_MODEL_DST)
            return _OG_MODEL_DST
        except OSError:
            return _OG_MODEL_SRC   # can't copy — use original in-place
    bundled = _res(os.path.join("models", "best.pt"))
