...
class RunTab(QWidget):
    ...
    def _build(self):
        ...
        br.addWidget(self.btn_start,2); br.addWidget(self.btn_master,1); br.addWidget(self.btn_flag)
        L.addLayout(br)
    ...
    def _upload_master(self):
        ...
        if choice == "picture":
            ...
            try:
                ...
                res=safe_predict(model, frame,conf=self._conf.value()/100,imgsz=640,verbose=False,device='cpu')[0]
                dets=[{"class":int(b.cls[0]),"name":model.names[int(b.cls[0])],
                       "xyxy":list(map(int,b.xyxy[0]))} for b in res.boxes]
                if not dets:
    ...
    def _start(self):
        ...
        self._cam.set_filters(self._pipe_filters)
    ...
    def on_project_changed(self, path):
        ...
        if os.path.exists(saved_pipe):
            try:
                ...
                self._log.append(f"[Project] Filters auto-loaded")
            except Exception as e:
                self._log.append(f"[Project] Filters load failed: {e}")
        else:
